...
const VERSION_HISTORY = [
    {
        ...
        date: "2026-08-02",
        changes: [
            "大改：progress 資料改成先依年份分層存放（progress/{年份}/{週}/{任務}），解決跨年後「今年第5週」跟「明年第5週」共用同一個 key 互相覆蓋的問題，改成直接分開存放，並自動遷移舊帳號的資料",
            "修正：加權模式比照編輯模式，切到年曆頁或時間線頁時會自動退出，回到主頁不會卡在加權比例調整畫面",
            "調整：刪除按鈕改用馬卡龍紅",
            "調整：電腦版「返回主頁」按鈕不顯示箭頭，年曆頁的按鈕文字改成「返回主頁」（跟時間線頁一致）",
            "調整：手機版返回鍵不再用 fixed 定位飄在畫面角落，改成留在原本的版面裡，跟標題同一行、垂直置中對齊",
            "調整：主頁臨時清單的「編輯模式」按鈕字樣改成「編輯」",
            "調整：編輯模式的操作按鈕群組加上靠右對齊",
            "修正：時間線手動記錄選不了已有分項的問題——原因是選分項時會誤觸重建分項選單的邏輯，把剛選的值洗掉，已修正",
            "新增大功能：臨時代辦資料夾——可以建立連動到某個大類別分項的資料夾，把臨時代辦拖曳進去，打勾完成時會自動幫連動的分項 +1",
            "調整：圓餅圖的滑鼠提示改成顯示「那一圈自己」的填滿百分比，不是整體倍率",
            "修正：圓餅圖第二、三圈不再有「整體達標才會突然出現一大塊」的跳動感，改成每個項目各自連續累積，跟第一圈一樣平滑"
        ]
    },
    {
        version: "v2026-08-01",
